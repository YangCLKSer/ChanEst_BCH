...
void EnsureCapacity_real(ARRAY_real* emxArray, int oldNumel)
{
    ...
    if (newNumel > emxArray->allocatedSize) {
        ...
        newData = calloc((unsigned int)i, sizeof(FLOAT_TYPE));
        if (emxArray->data != NULL) {
            memcpy(newData, emxArray->data, sizeof(FLOAT_TYPE) * oldNumel);
            if (emxArray->canFreeData) {
                free(emxArray->data);
            }
        }

        emxArray->data = (FLOAT_TYPE*)newData;
        emxArray->allocatedSize = i;
        emxArray->canFreeData = 1;
    }
}
...
void Free_real(ARRAY_real** pEmxArray)
{
    if (*pEmxArray != (ARRAY_real*)NULL) {
        if (((*pEmxArray)->data != (FLOAT_TYPE*)NULL) && (*pEmxArray)->canFreeData) {
            free((*pEmxArray)->data);
        }
