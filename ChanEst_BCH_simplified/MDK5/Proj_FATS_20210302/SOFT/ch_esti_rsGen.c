    ...
    int numRBDL, int numSymDL, int NCP)
{
    ...
    for (i = 0; i < loop_ub; i++) {
        RSSeq->data[i].re = sqrt_res * (FLOAT_TYPE)(1 - 2 * C->data[2 * i]);
        RSSeq->data[i].im = sqrt_res * (FLOAT_TYPE)(1 - 2 * C->data[2 * i + 1]);
    }*/
    ...
    sqrt_res = 0.70710678118654757;
    for (i = 0; i < loop_ub; i++) {
        valRS->data[i].re = sqrt_res * (FLOAT_TYPE)(1 - 2 * (C->data[2 * (mm->data[i]) - 2]));
        valRS->data[i].im = sqrt_res * (FLOAT_TYPE)(1 - 2 * (C->data[2 * (mm->data[i]) - 1]));
    }
